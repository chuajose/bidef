...
class Imap_mime {
	...
	function sq_sanitize($body,
	     ...
	     */
	    @array_walk($tag_list, array($this,'sq_casenormalize'));
	    @array_walk($rm_tags_with_content, array($this,'sq_casenormalize'));
	    @array_walk($self_closing_tags, array($this,'sq_casenormalize'));
	    /**
	    ...
	    while (($curtag = $this->sq_getnxtag($body, $curpos)) != FALSE){
	        ...
	        if ($tagname == "style" && $tagtype == 1){
	            list($free_content, $curpos) =
	                $this->sq_fixstyle($body, $gt+1, $message, $id);
	            if ($free_content != FALSE){
	                ...
	                $trusted .= $this->sq_tagprint($tagname, false, 2);
	            }
	            continue;
	        }
	                ...
	                if ($skip_content == false){
	                    ...
	                    if ($tagtype == 1 &&
	                        ...
	                        if (($rm_tags == false
	                            ...
	                            if ($tagname == "body"){
	                                $tagname = "div";
	                                $attary = $this->sq_body2div($attary,
	                                        $message, $id);
	                            }
	                            if ($tagtype == 1){
	                                if (isset($open_tags{$tagname})){
	 ...
	 */
	function magicHTML($id, $body, $take_mailto_links =false) {
		//$body = $this->get_mail($id);
		//$body = $body->textHtml;
		$message = $body;
	    ...
	    $add_attr_to_tag = Array(
	            "/^a$/i" =>
	            Array('target'=>'"_blank"',
	                'title'=>'"'._("Este enlace externo se abrirá en otra ventana").'"'
	                )
	            );
	 ...
	 * @param  $this->folder  the message mailbox
	...
	function sq_fixstyle($body, $pos, $message, $id){
	    ...
	    if (count($aMatch)) {
	        ...
	        foreach($aMatch[1] as $sMatch) {
	            // url value
	            $urlvalue = $sMatch;
	            $this->sq_fix_url('style',$urlvalue, $message, $id, "'");
	            $aValue[] = $sMatch;
	            $aReplace[] = $urlvalue;
	        }
	     ...
	     */
	    $contentTemp = $content;
	    $this->sq_defang($contentTemp);
	    $this->sq_unspace($contentTemp);
	...
	function sq_cid2http($message, $id, $cidurl){
	    ...
	    if (!empty($linkurl)) {
	        $httpurl = $quotchar . SM_PATH . 'src/download.php?absolute_dl=true&amp;' .
	            "passed_id=$id&amp;mailbox=" . urlencode($this->folder) .
	 ...
	 * @param  $this->folder  mailbox we're currently reading (for cid2http)
	 ...
	 */
	function sq_body2div($attary,$message, $id){
	    $me = 'sq_body2div';
	    ...
	    if (is_array($attary) && sizeof($attary) > 0){
	        foreach ($attary as $attname=>$attvalue){
	            ...
	            switch ($attname){
	                case 'background':
	                    $attvalue = $this->sq_cid2http($message, $id, $attvalue);
	...
	function sq_fixatts($tagname,
	    ...
	    while (list($attname, $attvalue) = each($attary)){
	        ...
	        if ($attname == 'style') {
	            ...
	            if (count($aMatch)) {
	                foreach($aMatch[1] as $sMatch) {
	                    // url value
	                    $urlvalue = $sMatch;
	                    $this->sq_fix_url($attname, $urlvalue, $message, $id, "'");
	         ...
	         */
	            ...
	            $this->sq_fix_url($attname, $attvalue, $message, $id);
	            $attary{$attname} = $attvalue;
	        }
	    }
	    /**
	 ...
	 */
	function sq_fix_url($attname, &$attvalue, $message, $id, $sQuote = '"') {
	    ...
	    $view_unsafe_images = TRUE;//cambio para poner siempre visibles las imagenes
	            ...
	            if (isset($aUrl['scheme'])) {
	                switch(strtolower($aUrl['scheme'])) {
	                    ...
	                    case 'outbind':
	                         ...
	                         */
	                        $attvalue = $sQuote . $this->sq_cid2http($message, $id, $attvalue) . $sQuote;
	                        break;
	                    case 'cid':
	                        /**
	...
	function find_ent_id($id, $message) {
	    for ($i = 0, $ret = ''; $ret == '' && $i < count($message->entities); $i++) {
	        if ($message->entities[$i]->header->type0 == 'multipart')  {
	            $ret = $this->find_ent_id($id, $message->entities[$i]);
	        } else {
